...
async function commandHeadingsHaveInlineCode (ast, file) {
  ...
  const validate = async (node) => {
    ...
    if (inlineCodeFound === false)
      file.message('Command heading without inline code', node)
  }

  await visit(ast, 'heading', validate)
}

module.exports = rule('remark-lint:no-dead-urls', commandHeadingsHaveInlineCode)
